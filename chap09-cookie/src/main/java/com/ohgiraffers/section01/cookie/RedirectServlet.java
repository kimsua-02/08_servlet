...
@WebServlet("/redirect") // redirect 를 받는쪽은 get 방식으로 받아야 함.
...
* 그렇기 때문에 개인 PC는 크게 상관이 없지만 공용 PC 등 다른 사용자와 함께 쓰는 컴퓨터이면 보안에 취약하다.
* 따라서 민감한 개인 정보를 취급하는 경우에는 쿠키보다는 세션을 이용한다.
* 세션은 쿠키와 유사한 형태로 저장되짐나 서버에서 관리되므로 보안에 더 우수하다는 장점을 가진다.
* */
// 로그인 정보는 보통 세션에 보관해서 서버 측에서 관리함. (보안상)
// 장바구니 같은 것은 쿠키에 저장된다. 그렇게 막 중요한 게 아니기 때문.
